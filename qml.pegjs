...
QMLFile
  = __ imports:QImportList? objects:QObjectList __ {
...
QImport
  = "import" _ name:$QIdentifier _ version:$[0-9\.]+ as:QImportAs? __ {
      return { name: name, version: version, as: as?as.name:null };
    }
  / "import" _ path:StringLiteral as:QImportAs? __ {
      return { path: path, as: as?as.name:null };
    }

QImportAs
  = _ "as" _ id:Identifier { return id; }

QObjectList
  = head:QObject tail:(__ QObject)* {
      var result = [head];
...
Applied
  = __ "on" __ list:AppliedPropertyList {
      return list;
  }

AppliedPropertyList
  = head:$([a-z] [a-zA-Z0-9_]*) __ tail:("," $([a-z] [a-zA-Z0-9_]*))* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][1]);
      }
...
QDefinition
  ...
  / d:QSignalDefinition { return {type: 'signals', value: d}; }
...
VarToken        = "var"        !IdentifierPart / "let"        !IdentifierPart
...
PrimaryExpression
  = ThisToken { return { type: "ThisExpression", name: "this" }; }
  / Identifier
  / Literal
  / ArrayLiteral
  / ObjectLiteral
